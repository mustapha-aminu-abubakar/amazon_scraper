...
def scrape_links(links: dict):
    ...
    cols = ['keyword', 'title', 'sale_price($)', 'anchor_price($)', 
            'ratings(5)', 'reviews', 'orders', 'scrape_datetime', 'url']  #product information
    ...
    for link in links:
        ...
        for product in products:
            ...
            try:
                href = product.find('a', class_='a-link-normal').get('href')
            except:
                href = ''
            ...
            product_info = [link.split('_')[0], 
                            ...
                            'https://www.amazon.com{}'.format(href.split('?')[0])]              #adding product info to dataframe
...
def save_dataframe(dframe, filename: str):
    ...
    num_cols = [col for col in df.columns if col not in ['keyword', 'title', 'orders', 'scrape_datetime', 'url']]             #highlighting numerical attributes
